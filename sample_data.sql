...
--CREATE GEOMETRIES FROM X Y POINTS
...
-- CREATE POI TABLE (FIRE EXTENGUISHERS)
...
-- create geom from x y
UPDATE pointsOfInterest SET the_geom = st_makePoint(x,y);

-snap the points to nearest line of same floor and store the new geom in column newPoint
...
--CREATE TOPOLOGOGY (This will fill the source and target columns of edges)
SELECT pgr_createTopology('sample_edge_table',0.001);
